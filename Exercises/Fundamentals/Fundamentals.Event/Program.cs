 ...
 */
pubsub.OnChangeWithParam +=
    reportServer.PrecalculateStatisticsForUser; // Can subscribe more than 1 method to same event
...
namespace Fundamentals.Event
{
    public class Pubsub
    {
        ...
        public event Action<Pubsub, string>?
            OnChangeWithParam; // OnChangeWithObject?.Invoke(this) - this one will pass reference to self when invoked
        ...
        public void HandleUserConnect()
        {
            ...
            UsersOnline++;
            OnChange?.Invoke();
            OnChangeWithParam?.Invoke(this,
                "Chris"); // OnChange?. makes sure event is only invoked if there are subscribers
        }
    }
